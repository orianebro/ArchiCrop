...
def compute_continuous_element_with_constraint(
    element_node, time, constraint, classic=False
):  # see maybe with *kwarg, **kwds, etc. for time
    """compute geometry of Adel base elements (LeafElement and StemElement)
    element_node should be a mtg node proxy"""
    n = element_node
    ...
    if n.start_tt <= time < n.end_tt:  # organ growing
        ...
        # constraint = [height, leaf area] for a given plant
        #relative_growth = (time - n.start_tt) / (n.end_tt - n.start_tt)
        if n.label.startswith("Leaf") or n.label.startswith("Stem"):
            n.visible_length += constraint
            n.grow = True
...
class CerealsVisitorConstrained(CerealsVisitor):
    ...
    def __call__(self, g, v, turtle, time, constraint):
        # 1. retrieve the node
        n = g.node(v)
        ...
        if n.label.startswith("Leaf") or n.label.startswith("Stem"):
            ...
            # if n.length > 0:
            # print(v)
            mesh = compute_continuous_element_with_constraint(n, time, constraint, self.classic)
            if mesh:  # To DO : reset to None if calculated so ?
                n.geometry = turtle.transform(mesh)
                n.anchor_point = turtle.getPosition()
